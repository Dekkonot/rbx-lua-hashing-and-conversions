...
local INIT_0_224 = 0xc1059ed8
local INIT_1_224 = 0x367cd507
local INIT_2_224 = 0x3070dd17
local INIT_3_224 = 0xf70e5939
local INIT_4_224 = 0xffc00b31
local INIT_5_224 = 0x68581511
local INIT_6_224 = 0x64f98fa7
local INIT_7_224 = 0xbefa4fa4
...
if ASSERTIONS_ENABLED then
    ...
    assert(sha256("The Fitness-Gram Pacer Test is a multi-stage aerobic capacity test") == "8509e3a2478a4b7fb712420a8bc4177c2271de6122b0d8bcb792c2ba4203a330", "(SHA-256) Fitness-Gram hash does not match")
    if true then
    ...
    -- SHA-224 tests
    assert(sha224("abc") == "23097d223405d8228642a477bda255b32aadbce4bda0b3f7e36c9da7", "(SHA-224) abc hash does not match")
    assert(sha224("") == "d14a028c2a3a2bc9476102bb288234c415a2b01f828ea62ac5b3e42f", "(SHA-224) empty hash does not match")
    assert(sha224("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq") == "75388b16512776cc5dba5da1fd890150b0c6455cb4f58b1952522525", "(SHA-224) 448 bit alphabet hash does not match")
    assert(sha224("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu") == "c97ca9a559850ce97a04a96def6d99a9e0e0e2ab14e6b8df265fc0b3", "(SHA-224) 896 bit alphabet hash does not match")
    ...
    assert(sha224("The Fitness-Gram Pacer Test is a multi-stage aerobic capacity test") == "795f46c7974e93ca25ba2abf56525c5a234563b43b774904bc25fd27", "(SHA-224) Fitness-Gram hash does not match")
    if true then
        assert(sha224(string.rep("e", 199999)) == "386146cd6a85d13e8d66b8131bbb6c61e08d84a514ae845c59c13fd0", "(SHA-224) e hash does not match")
        assert(sha224(string.rep("a", 1e6)) == "20794655980c91d8bbb4c1ea97618a4bf03f42581948b2ee4ee7ad67", "(SHA-224) million a hash does not match")
    end

    print("SHA-256/224 tests completed. Took", tick()-t)
end
